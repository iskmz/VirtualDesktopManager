...
namespace VirtualDesktopManager
{
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - //
    // at 2022-10-02: re-copied the updated Version 1.10, 2022-07-29 , from https://github.com/MScholtes/VirtualDesktop/blob/master/VirtualDesktop.cs //
    // this version is compatible with  Windows 10 1809 to 21H2  ; also note that there is NO setname option for previous versions of win10 // 
    // kept for backwards compatibility with win10 // but not needed (nor used) in win11 //
    // note that setname() function was fixed to adapt it to .net-core-5: HSTRING data type not found there; so was taking from some other .dll //
    ...
    */
    ...
    internal interface IVirtualDesktopManagerInternal2
    {
        ...
        // void SetName(IVirtualDesktop desktop, [MarshalAs(UnmanagedType.HString)] string name); // not working on .net-core-5 (runtime error) // fix: line below
        void SetName(IVirtualDesktop desktop, HSTRING name); // HSTRING type is used from winRT .. .dll below // 
    }

    // [code-snippet] added 2022-10-02 ; to adapt for .net-core-5  & also keep backward compatibility with win10 
    // to be used with modified SetName() above
    // credits/idea to:  https://github.com/dotnet/runtime/issues/39827#issuecomment-663265639  
    // - - - -  - - - -  - - - -  - - - -  - - - -  - - - -  - - - -  - - - -  - - - -  - - - - //
    ...
    struct HSTRING
    {
        ...
        // the following dll is added with release binaries; just in case its missing in the system //
    ...
    static class AHK
    {
        ...
        {
            // below line was added at 2022-10-02,  to adapt code to .net-core-5 to be used on win11 ; and also to keep backwards compatibility for win10 //
            string msvd_version = Environment.OSVersion.Version.Build >= 22000 ? "msvd11.exe" : "msvd10.exe";

            string script = "#NoTrayIcon\n"
                + "; Below line re-maps WIN+CTRL+LEFT-ARROW\n"
                + "#^Left::Run, " + msvd_version + " /Quiet /Wrap /Left , , Hide\n"
                + "return\n"
                + "; Below line re-maps WIN+CTRL+RIGHT-ARROW\n"
                + "#^Right::Run, " + msvd_version + " /Quiet /Wrap /Right , , Hide\n"
                + "return\n"
            ...
            // MessageBox.Show(script); // to check code
        }

        // modified to AutoHotkey64.dll on 2022-10-02 , to adapt code to .net core 5 //
        // i.e. downloaded and used the 64bit version of autohotkey found on: https://github.com/HotKeyIt/ahkdll-v1-release/tree/master/x64w //

        [DllImport("AutoHotkey64.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Unicode, EntryPoint = "ahkdll")]
        ...
        [DllImport("AutoHotkey64.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Unicode, EntryPoint = "ahktextdll")]
