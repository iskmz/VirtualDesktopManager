...
namespace VirtualDesktopManager
{
    public partial class Form1 : Form
    {
        ...
        // as of version 2.4.1-w11 (for windows 11) , which uses VirtualDesktop.dll v5.0.5 (& .net core 5)
        // there is no need for MScholtes repo. used below (in the first ELSE branch; kept for backwards compatibility with win10)
        // because v5.0.5 has Name feature for VirtualDesktop (works only with win11)
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - //
        // added 2022-03-02
        ...
        {
            // MessageBox.Show(Environment.OSVersion.Version.Build + ""); // for checking // 
            if (Environment.OSVersion.Version.Build >= 22000) // if win 11; "if" was added @ 2022-10-02 , to add support for win11 & v5.0.5 of VirtualDesktop.dll
            {
                desktops[index].Name = name; // not supported on win 10 
                return true;
            }
            else // win 10 
            {
                ...
                {
                    // next line changed , to adapt to .net-core-5 // changed (name) to HSTRING.FromString(name) // 2022-10-02
                    VDMInternal2.SetName(VDMInternal2.FindDesktop(desktops.ElementAt(index).Id), HSTRING.FromString(name));
                    return true;
                }
                return false;
            }
        ...
        {
            var folderBrowserDialog = new FolderBrowserDialog  // changed on 2022-10-02 ;  to adapt code to .net core 5 //
            {
